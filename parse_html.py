...
用法:
    python3 parse_html.py <html文件路径>

示例:
    python3 parse_html.py rec/response_body.html
    python3 parse_html.py data/response_body.html
    python3 parse_html.py /path/to/your/file.html
"""
...
def main():
    ...
    if len(sys.argv) < 2:
        print('=' * 60)
        print('❌ 错误：缺少HTML文件路径参数')
        print('=' * 60)
        print('\n用法:')
        print(f'  python3 {sys.argv[0]} <html文件路径>')
        print('\n示例:')
        print(f'  python3 {sys.argv[0]} rec/response_body.html')
        print(f'  python3 {sys.argv[0]} data/response_body.html')
        print(f'  python3 {sys.argv[0]} /path/to/your/file.html')
        sys.exit(1)

    input_file = Path(sys.argv[1])

    print('=' * 60)
    print(f'解析 {input_file} 并更新数据库')
    print('=' * 60)
    print()
    ...
    print('\n难度分布:')
    diff_order = ['Basic', 'Advanced', 'Expert', 'Master', 'Re:Master', 'utage']
    for diff in diff_order:
        if diff in stats['difficulties']:
            print(f"  {diff}: {stats['difficulties'][diff]}")

    print('\n评级分布:')
    rating_order = ['SSSPLUS', 'SSS', 'SSPLUS', 'SS', 'S', 'AAA', 'AA', 'A']
