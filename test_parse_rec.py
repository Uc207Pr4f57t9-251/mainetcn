...
# -*- coding: utf-8 -*-
"""
解析 rec/response_body.html 并生成数据库
用于测试最新上传的HTML文件
"""
...
class Config:
    ...
    INPUT_HTML = REC_DIR / 'response_body.html'
...
def main():
    print('=' * 60)
    print('解析 rec/response_body.html 并生成数据库')
    print('=' * 60)
    print()

    if not Config.INPUT_HTML.exists():
        print(f"❌ 文件不存在: {Config.INPUT_HTML}")
        return
    ...
    # 步骤2：读取和解析HTML
    print('步骤2: 读取和解析 rec/response_body.html')
    with open(Config.INPUT_HTML, 'r', encoding='utf-8') as f:
        html_content = f.read()
